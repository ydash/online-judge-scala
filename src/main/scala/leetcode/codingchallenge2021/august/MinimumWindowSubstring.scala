...
object MinimumWindowSubstring {
  ...
  def minWindow(s: String, t: String): String = {
    ...
    sliding(0, 0, filteredS, count, None)
      .map(range => s.substring(range.start, range.last + 1))
      .getOrElse("")
  }
  ...
  private def sliding(
      left: Int,
      right: Int,
      seq: IndexedSeq[(Char, Int)],
      count: mutable.Map[Char, Int],
      acc: Option[Range]
  ): Option[Range] =
    if (right >= seq.length) acc
      ...
      val minRange = (seq(l)._2 to i).pipe(rng =>
        if (
          count.values.forall(_ <= 0) && rng.length < acc
            .map(_.length)
